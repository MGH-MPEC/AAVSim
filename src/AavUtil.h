...
/* 
    AavUtil contains the platform specific utility functions for handling files/directories, 
...
public:
    ...
    /* Constant values for AAVSim version and file/directory information */
    static const char *AAVSim_INPUT_VERSION;
    static const char *AAVSim_VERSION_STRING;
    static const char *AAVSim_EXECUTABLE_COMPILED_DATE;
	...
	std::string resultsSubDirectory;
	...
	//static void createResultsSubDirectory(string dirName);
	//static void changeDirectoryToResultsSubDirectory();
	//static void createTraceFilesDirectory();
	//static void changeDirectoryToTraceFilesDirectory();
    ...
    /* Probability modification functions - DO WE NEED THIS?*/
 ...
 **/
inline void AavUtil::setRandomSeedType(bool useTimeSeed) {
 ...
 *
 */
inline void AavUtil::setFixedSeed(Patient *patient){
 ...
 **/
inline double AavUtil::getRandomDouble() {
	return mtRand();
} /* end getRandomDouble */
 ...
 **/
inline double AavUtil::getRandomGaussian(double mean, double stdDev, int callSiteId, Patient *patient) {
 ...
 **/
inline double AavUtil::probToRate(double prob) {
	return (-1 * log(1 - prob));
} /* end probToRate */
 ...
 **/
inline double AavUtil::rateToProb(double rate) {
	return (1 - exp(-1 * rate));
} /* end rateToProb */
 ...
 **/
inline double AavUtil::probRateMultiply(double prob, double rateMult) {
 ...
 **/
inline double AavUtil::probToLogit(double prob) {
 ...
 * \return a double representing the equivalent probability
 **/
inline double AavUtil::logitToProb(double logit) {
 ...
 **/
inline double AavUtil::probLogitAdjustment(double prob, double logitAdjust) {
	return logitToProb(probToLogit(prob) + logitAdjust);
}

inline int selectBasedOnOdds(std::vector<float>& odds, float randNum) {
    ...
    return -1; // if nothing happens, return -1 to indicate error occurred
}